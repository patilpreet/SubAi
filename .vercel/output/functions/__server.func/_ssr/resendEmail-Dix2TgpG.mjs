...
//#region node_modules/.nitro/vite/services/ssr/assets/resendEmail-Dix2TgpG.js
var RESEND_FROM = "SubAI <noreply@sub-ai-iota.vercel.app>";
var rateLimitStore = {};
function checkRateLimit(identifier, maxRequests = 5, windowMs = 6e4) {
	const now = Date.now();
	const windowStart = now - windowMs;
	if (!rateLimitStore[identifier]) rateLimitStore[identifier] = [];
...
var sendWelcomeEmail_createServerFn_handler = createServerRpc({
	id: "a147262f7d8bff0566996274a8d381519f557d9bdf90a91291809184d4b4530a",
	name: "sendWelcomeEmail",
	filename: "src/lib/resendEmail.js"
...
}).handler(sendWelcomeEmail_createServerFn_handler, async ({ data }) => {
...
var sendTestEmail_createServerFn_handler = createServerRpc({
	id: "0fcd7c59fb9dbafd8bac6460d9266399c12dc620e3e9bf0884d64e52d7fdaa97",
	name: "sendTestEmail",
	filename: "src/lib/resendEmail.js"
}, (opts) => sendTestEmail.__executeServer(opts));
var sendTestEmail = createServerFn({ method: "POST" }).validator((input) => {
	const payload = input?.data ? input.data : input;
	if (!payload || typeof payload.email !== "string") throw new Error("email is required");
	return {
		email: payload.email,
...
}).handler(sendTestEmail_createServerFn_handler, async ({ data }) => {
	const rateCheck = checkRateLimit(`test-${data.email}`, 3, 6e4);
	if (!rateCheck.success) return {
		success: false,
	...
	const result = await postEmail({
		...
		html: emailLayout({
          ...
          <p style="font-size: 12px; color: #52525b; text-align: center; margin: 20px 0 0;">
            Sent ${(/* @__PURE__ */ new Date()).toLocaleDateString("en-IN", {
				weekday: "long",
				year: "numeric",
				month: "long",
				day: "numeric",
				timeZone: "Asia/Kolkata"
			})} at ${(/* @__PURE__ */ new Date()).toLocaleTimeString("en-IN", {
				hour: "2-digit",
				minute: "2-digit",
				timeZone: "Asia/Kolkata",
				timeZoneName: "short"
...
var sendBudgetAlertEmail_createServerFn_handler = createServerRpc({
	id: "678becf78b64b91e0ed14096a87890e2afd868e8a7994bdb056ce7ea7c6097ea",
	name: "sendBudgetAlertEmail",
	filename: "src/lib/resendEmail.js"
}, (opts) => sendBudgetAlertEmail.__executeServer(opts));
var sendBudgetAlertEmail = createServerFn({ method: "POST" }).validator((input) => {
	const payload = input?.data ? input.data : input;
...
}).handler(sendBudgetAlertEmail_createServerFn_handler, async ({ data }) => {
	const rateCheck = checkRateLimit(`budget-${data.email}`, 3, 6e4);
	if (!rateCheck.success) return {
		success: false,
		message: rateCheck.message
	};
	...
	const result = await postEmail({
		from: RESEND_FROM,
		to: [data.email],
		subject: isOverBudget ? `\u26A0\uFE0F Budget exceeded: ${category}` : `\u26A0\uFE0F Budget alert: ${category} at ${percentage}%`,
		html: emailLayout({
			previewText: isOverBudget ? `Your ${category} budget has been exceeded.` : `Your ${category} budget is at ${percentage}%.`,
			children: `
          ...
          <div style="background: ${isOverBudget ? "rgba(239, 68, 68, 0.08)" : "rgba(217, 119, 6, 0.08)"}; border: 1px solid ${isOverBudget ? "rgba(239, 68, 68, 0.2)" : "rgba(217, 119, 6, 0.2)"}; border-radius: 8px; padding: 16px; text-align: center;">
            <p style="margin: 0; font-size: 13px; color: ${isOverBudget ? "#fca5a5" : "#fde68a"};">
              ${isOverBudget ? `You've spent <strong>Rs.${spent.toLocaleString("en-IN")}</strong> — Rs.${(spent - limit).toLocaleString("en-IN")} over your <strong>${category}</strong> budget.` : `You've used <strong>${percentage}%</strong> of your <strong>${category}</strong> budget (Rs.${spent.toLocaleString("en-IN")} of Rs.${limit.toLocaleString("en-IN")}).`}
            </p>
          </div>
        `
		})
...
//#endregion
export { sendBudgetAlertEmail_createServerFn_handler, sendTestEmail_createServerFn_handler, sendWelcomeEmail_createServerFn_handler };
