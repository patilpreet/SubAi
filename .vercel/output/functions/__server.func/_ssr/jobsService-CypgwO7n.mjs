...
//#region node_modules/.nitro/vite/services/ssr/assets/jobsService-CypgwO7n.js
/**
...
*/
async function createJob({ title, language = "hinglish", duration = "—", storageKey, aiDescription }) {
	const { data: { user } } = await supabase.auth.getUser();
...
*/
async function getVideoUrl(storageKey) {
...
*/
async function saveSubtitles(jobId, subtitles) {
	const { data: { user } } = await supabase.auth.getUser();
	if (!user) return;
...
async function loadSubtitles(jobId) {
	const { data, error } = await supabase.from("subtitles").select("*").eq("job_id", jobId).order("sort_order", { ascending: true });
	if (error) {
...
//#endregion
export { getVideoUrl as a, uploadVideo as c, fetchJobs as i, createJob as n, loadSubtitles as o, deleteJob as r, saveSubtitles as s, completeJob as t };
