//#region node_modules/.nitro/vite/services/ssr/assets/scriptConverter-j65-djWY.js
/**
...
function devanagariToHinglish(text) {
	if (!text) return "";
	let result = text;
	for (const [dev, hing] of Object.entries(DEVANAGARI_WORDS_MAP)) result = result.split(dev).join(hing);
	let transliterated = "";
	const chars = Array.from(result);
	for (let i = 0; i < chars.length; i++) {
		const ch = chars[i];
		const nextCh = chars[i + 1];
		if (DEV_CHAR_MAP[ch] !== void 0) {
			let roman = DEV_CHAR_MAP[ch];
			const isConsonant = /[\u0915-\u0939\u0958-\u095F]/.test(ch);
			const isNextMatraOrVirama = /[\u093E-\u094D\u0962\u0963]/.test(nextCh || "");
			if (isConsonant && !isNextMatraOrVirama && nextCh && /[^\s\p{P}]/u.test(nextCh)) roman += "a";
...
*/
function hinglishToEnglish(text) {
	let result = text;
...
*/
function capitalizeSegment(text) {
	if (!text) return "";
...
*/
function convertSubtitles(subtitles, script) {
	if (!Array.isArray(subtitles)) return [];
	return subtitles.map((s) => {
		let newText = s.text;
		if (script === "roman") {
			if (/[\u0900-\u097F]/.test(s.text)) newText = devanagariToHinglish(s.text);
		} else if (script === "native") newText = hinglishToDevanagari(s.text);
		else if (script === "english") newText = hinglishToEnglish(s.text);
		return {
			...s,
			text: capitalizeSegment(newText)
		};
	});
}
//#endregion
export { devanagariToHinglish as n, convertSubtitles as t };
