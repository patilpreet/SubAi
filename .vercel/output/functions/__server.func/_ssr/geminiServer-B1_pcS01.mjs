...
//#region node_modules/.nitro/vite/services/ssr/assets/geminiServer-B1_pcS01.js
...
*/
var transcribeWithGemini_createServerFn_handler = createServerRpc({
	id: "27286d7af3dc1deaa6a99a358bf8caea07d13e225c418c545a163a0e2aef4ef7",
	name: "transcribeWithGemini",
	filename: "src/lib/geminiServer.js"
}, (opts) => transcribeWithGemini.__executeServer(opts));
...
}).handler(transcribeWithGemini_createServerFn_handler, async ({ data }) => {
...
Language context: ${data.language === "hinglish" ? "Hinglish (a mix of Hindi and English spoken in Roman script)" : data.language === "hindi" ? "Hindi (Devanagari script, transliterated to Roman)" : data.language === "english" ? "English" : data.language}
...
[{"start":0.0,"end":1.8,"text":"toh basically kya hai na"},{"start":1.8,"end":3.5,"text":"hum log jo hai wo"},{"start":3.5,"end":5.2,"text":"ek naya project bana rahe hain"}]`;
	const apiResponse = await fetchWithTimeout(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${apiKey}`, {
		method: "POST",
	...
	const subtitles = arr.filter((item) => item && typeof item.start === "number" && typeof item.end === "number" && item.text).map((item) => {
		let segText = String(item.text).trim();
		if ((!data.language || data.language === "hinglish" || data.language === "auto") && /[\u0900-\u097F]/.test(segText)) segText = devanagariToHinglish(segText);
		return {
			id: Math.random().toString(36).substring(2, 9),
			start: Math.max(0, Number(item.start)),
			end: Math.max(Number(item.start) + .1, Number(item.end)),
			text: segText
		};
...
//#endregion
export { transcribeWithGemini_createServerFn_handler };
