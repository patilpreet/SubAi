...
const fetchWithTimeout = async (url, options = {}, timeoutMs = 60000) => {
  ...
  try {
    const res = await __fetch(url, { ...options, signal: controller.signal });
    return res;
  } finally {
    clearTimeout(timer);
  }
...
export const transcribeWithSarvam = createServerFn({ method: "POST" })
  .validator((input) => {
  ...
  .handler(async ({ data }) => {
    ...
    if (!apiKey) {
      ...
      return { ok: false, subtitles: [], error: "SARVAM_API_KEY missing in .env" };
    }
    ...
    // SarvamAI rejects "video/mp4" but accepts "audio/mp4"
    ...
    if (!sarvamRes.ok) {
      ...
      return { ok: false, subtitles: [], error: `SarvamAI sync API failed (${sarvamRes.status}): ${errText.slice(0, 300)}` };
    }
    ...
    // Normalize words from any possible response shape
    ...
    if (Array.isArray(rawWords) && rawWords.length > 0) {
      // Array of objects: [{ word, start, end, start_time_seconds, end_time_seconds }]
      wordEntries = rawWords
        .map((w) => {
      ...
      // { words: string[], start_time_seconds: number[], end_time_seconds: number[] }
      const ts = timestamps;
    ...
    } else if (timestamps && Array.isArray(timestamps)) {
      // Direct array of timing objects: [{ start, end, text }]
      wordEntries = timestamps
        .map((t) => {
    ...
    const errorMsg =
      subtitles.length === 0
        ? `SarvamAI returned no captions. Transcript: ${JSON.stringify(result.transcript)}. Keys: ${Object.keys(result).join(", ")}`
        : null;
