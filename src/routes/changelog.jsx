    ...
    <Layout>
      <section className="pt-36 pb-24 px-6">
        <div className="max-w-2xl mx-auto">
          ...
          <p className="text-[var(--text-secondary)] mb-12">Every update to SubAI, in one place.</p>

          <div className="space-y-10">
            {CHANGES.map((release) => (
              <div key={release.version} className="border-l-2 border-[var(--primary)]/30 pl-6 relative">
                <div className="absolute left-0 top-0 -translate-x-[5px] w-2 h-2 rounded-full bg-[var(--primary)]" />
                <div className="flex items-center gap-3 mb-3">
                  <span className="text-lg font-bold text-white">v{release.version}</span>
                  <span className="text-xs text-[var(--text-secondary)]">{release.date}</span>
                </div>
